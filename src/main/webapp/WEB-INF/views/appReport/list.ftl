					...
					<div class="page-content" >
						<div class="page-head">
							<div class="page-title">
								<h1><i class="icon-grid" style="font-size: 20px;"></i>&nbsp;app用户行为数据统计表管理   </h1>
							</div>
						...
						<ul class="page-breadcrumb breadcrumb">
							...
							<li>
								<a href="javascript:;">app用户行为数据统计表管理</a>
							</li>
						</ul>
							<div class="row">
								<div class="col-md-12">
									<div class="portlet light bordered">
										<div class="portlet-title">
											<div class="caption">
												<i class="fa fa-globe"></i>app用户行为数据统计表管理
											</div>
											<div class="actions">
												<a href="${base}/admin/appReport/add.jhtml"  class="btn btn-default btn-sm "> <i class="fa fa-plus"></i>添加</a>
											...
											<div class="portlet-body">
												...
												<div id="sample_1_wrapper" class="dataTables_wrapper no-footer"><div class="table-scrollable">
													<table class="table table-striped table-bordered table-hover dataTable no-footer" id="table_list" role="grid" aria-describedby="sample_2_info">
														<thead>
															<tr role="row">
																<th class="table-checkbox sorting_disabled" rowspan="1" colspan="1" style="width: 24px;">
				                                                 ...
				                                                 <th >用户id</th> 
				                                                 <th >设备uuid</th> 
				                                                 <th >操作类型(0-登录,1-首页停留,2-直播间停留,3-签到)</th> 
				                                                 <th >停留时间(如首页停留、直播间停留)</th> 
				                                                 <th >登录方式(游客、账号、QQ(web/app)、微博(web/qq)、微信)</th> 
				                                                 <th >登录时间</th> 
				                                                 <th >注册时间</th> 
				                                                 <th >app应用安装时间</th> 
				                                                 <th >记录添加时间</th> 
														...
														<tbody>
															[#list page.content as appReport]
																<tr class="gradeX [#if (appReport_index+1) % 2 != 0]odd[#else]even[/#if]" role="row">
																	<td>
																		<span><input type="checkbox" name="ids" class="checkboxes" value="${appReport.id}" ></span>
																	</td>
				                                                    <td>${ appReport.id}</td>
				                                                    
				                                                    <td>${ appReport.userId}</td>
				                                                    
				                                                    <td>${ appReport.uuid}</td>
				                                                    
				                                                    <td>${ appReport.operateType}</td>
				                                                    
				                                                    <td>${ appReport.stayTime}</td>
				                                                    
				                                                    <td>${ appReport.loginType}</td>
				                                                    
				                                                    <td>${ appReport.loginTime}</td>
				                                                    
				                                                    <td>${ appReport.registeTime}</td>
				                                                    
				                                                    <td>${ appReport.appinstallTime}</td>
				                                                    
				                                                    <td>${ appReport.createTime}</td>
				                                                    
																	<td style="width: 125px;">
																		<a href="${base}/admin/appReport/edit/${appReport.id}.jhtml" class="btn default btn-xs purple "> <i class="fa fa-edit"></i> 编辑</a>
																		<a href="${base}/admin/appReport/view/${appReport.id}.jhtml" class="btn default btn-xs green-stripe ">查看</a>
																	</td>
																</tr>
															[/#list]
														</tbody>
