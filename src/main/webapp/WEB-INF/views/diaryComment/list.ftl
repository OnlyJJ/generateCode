					...
					<div class="page-content" >
						<div class="page-head">
							<div class="page-title">
								<h1><i class="icon-grid" style="font-size: 20px;"></i>&nbsp;动态评论表管理   </h1>
							</div>
								...
								<div class="col-md-12">
									<div class="portlet light bordered">
										<div class="portlet-title">
											<div class="caption">
												<i class="fa fa-globe"></i>动态评论表管理
											</div>
											<div class="actions">
												<a href="${base}/admin/diaryComment/add.jhtml"  class="btn btn-default btn-sm "> <i class="fa fa-plus"></i>添加</a>
											...
											<div class="portlet-body">
												...
												<div id="sample_1_wrapper" class="dataTables_wrapper no-footer"><div class="table-scrollable">
													<table class="table table-striped table-bordered table-hover dataTable no-footer" id="table_list" role="grid" aria-describedby="sample_2_info">
														<thead>
															<tr role="row">
																<th class="table-checkbox sorting_disabled" rowspan="1" colspan="1" style="width: 24px;">
				                                                 ...
				                                                 <th ></th> 
				                                                 <th >动态id</th> 
				                                                 <th >评论回复类型,0:回复整个动态;1:回复某个评论</th> 
				                                                 <th >评论id,为空表示回复整个评论,不为空表示回复某条评论</th> 
				                                                 <th ></th> 
				                                                 <th >评论文字内容</th> 
				                                                 <th ></th> 
				                                                 <th ></th> 
				                                                 ...
				                                                 <th >管理状态,0:默认系统自审通过,1:被举报违规而删除,2:人工审核通过</th> 
														...
														<tbody>
															[#list page.content as diaryComment]
																<tr class="gradeX [#if (diaryComment_index+1) % 2 != 0]odd[#else]even[/#if]" role="row">
																	<td>
																		<span><input type="checkbox" name="ids" class="checkboxes" value="${diaryComment.id}" ></span>
																	</td>
				                                                    <td>${ diaryComment.id}</td>
				                                                    
				                                                    <td>${ diaryComment.userId}</td>
				                                                    
				                                                    <td>${ diaryComment.diaryInfoId}</td>
				                                                    
				                                                    <td>${ diaryComment.commentType}</td>
				                                                    
				                                                    <td>${ diaryComment.diaryCommentId}</td>
				                                                    
				                                                    <td>${ diaryComment.commentTime}</td>
				                                                    
				                                                    <td>${ diaryComment.commentTextInfo}</td>
				                                                    
				                                                    <td>${ diaryComment.ip}</td>
				                                                    
				                                                    <td>${ diaryComment.address}</td>
				                                                    
				                                                    <td>${ diaryComment.clientType}</td>
				                                                    
				                                                    <td>${ diaryComment.isUserCancel}</td>
				                                                    
				                                                    <td>${ diaryComment.mgrState}</td>
				                                                    
																	<td style="width: 125px;">
																		<a href="${base}/admin/diaryComment/edit/${diaryComment.id}.jhtml" class="btn default btn-xs purple "> <i class="fa fa-edit"></i> 编辑</a>
																		<a href="${base}/admin/diaryComment/view/${diaryComment.id}.jhtml" class="btn default btn-xs green-stripe ">查看</a>
																	</td>
																</tr>
															[/#list]
														</tbody>
