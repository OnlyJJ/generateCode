								...
								<div class="col-md-12">
									<div class="portlet light bordered">
										<div class="portlet-title">
											...
											<div class="actions">
												<a href="${base}/admin/pushConfig/add.jhtml"  class="btn btn-default btn-sm "> <i class="fa fa-plus"></i>添加</a>
											...
											<div class="portlet-body">
												...
												<div id="sample_1_wrapper" class="dataTables_wrapper no-footer"><div class="table-scrollable">
													<table class="table table-striped table-bordered table-hover dataTable no-footer" id="table_list" role="grid" aria-describedby="sample_2_info">
														<thead>
															<tr role="row">
																<th class="table-checkbox sorting_disabled" rowspan="1" colspan="1" style="width: 24px;">
				                                                 ...
				                                                 <th ></th> 
				                                                 <th >app应用注册信鸽生成id</th> 
				                                                 <th >app应用注册信鸽生成的密钥</th> 
				                                                 <th >客户端类型 3-IOS；0-Android；默认0</th> 
				                                                 <th >app包名</th> 
				                                                 <th >启用状态，1-启用，0-停用</th> 
				                                                 <th >添加时间</th> 
														...
														<tbody>
															[#list page.content as pushConfig]
																<tr class="gradeX [#if (pushConfig_index+1) % 2 != 0]odd[#else]even[/#if]" role="row">
																	<td>
																		<span><input type="checkbox" name="ids" class="checkboxes" value="${pushConfig.id}" ></span>
																	</td>
				                                                    <td>${ pushConfig.id}</td>
				                                                    
				                                                    <td>${ pushConfig.accessId}</td>
				                                                    
				                                                    <td>${ pushConfig.secretKey}</td>
				                                                    
				                                                    <td>${ pushConfig.appType}</td>
				                                                    
				                                                    <td>${ pushConfig.pckName}</td>
				                                                    
				                                                    <td>${ pushConfig.useStatus}</td>
				                                                    
				                                                    <td>${ pushConfig.addTime}</td>
				                                                    
																	<td style="width: 125px;">
																		<a href="${base}/admin/pushConfig/edit/${pushConfig.id}.jhtml" class="btn default btn-xs purple "> <i class="fa fa-edit"></i> 编辑</a>
																		<a href="${base}/admin/pushConfig/view/${pushConfig.id}.jhtml" class="btn default btn-xs green-stripe ">查看</a>
																	</td>
																</tr>
															[/#list]
														</tbody>
